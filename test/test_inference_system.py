...
IMAGES_PATH = 'datasets/ISIC2018_Task1-2_Test_Input'
DEVICE = 'cuda'
VIZ_OUTPUT_PATH = 'output/inference_result.jpg'
JSON_OUTPUT_PATH = 'output/test/inference_test_result.json'
REPORT_OUT_FOLDER = 'output/report'
DESCRIPTION_OUT_FOLDER = 'output/description'
VIZ_OUT_FOLDER = 'output/viz'
...
def main():
    ...
    for p in images_path.rglob("*.jpg"):
        images.append({
            "path": str(p),
            "id": p.stem  # filename without .jpg
        })

    # example: inspect first few
    print(f"Total images: {len(images)}")
    
    Path(REPORT_OUT_FOLDER).mkdir(parents=True, exist_ok=True)
    Path(DESCRIPTION_OUT_FOLDER).mkdir(parents=True, exist_ok=True)
    Path(VIZ_OUT_FOLDER).mkdir(parents=True, exist_ok=True)
    ...
    for image in tqdm(images[:100]):
        cls = inference_system.classification_inference.infer(image_path=image['path'])
        pred_text = cls['prediction_text']
        cls_count[pred_text] = cls_count.get(pred_text, 0) + 1
        
        if cls_count[pred_text] > max_per_cls :
            print(f"Skipping image {image['id']} for class {pred_text} (max per class reached)")
            continue
        
        for ablation_type in InferenceSystem.ABLATION_TYPES :
            results = inference_system.infer(image['path'], ablation_type=ablation_type)
            ...
            # Save report and desc as txt
            report_path = f'{REPORT_OUT_FOLDER}/{image["id"]}.txt'
            with open(report_path, "w", encoding="utf-8") as f:
                f.write(out['report'])
            
            description_path = f'{DESCRIPTION_OUT_FOLDER}/{image["id"]}.txt'
            with open(description_path, "w", encoding="utf-8") as f:
                f.write(out['description'])
            
            # Save visualization    
            visualization = results['visualization']
            visualization_bgr = cv2.cvtColor(visualization, cv2.COLOR_RGB2BGR)
            viz_path = f'{VIZ_OUT_FOLDER}/{image["id"]}_abl-{ablation_type}.png'
            cv2.imwrite(viz_path, visualization_bgr)
